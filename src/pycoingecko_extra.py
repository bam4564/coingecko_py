...
def method_queueable(self, fn, *args, **kwargs):
    ...
    if qid:
        ...
        if qid in self._queued_calls: 
            logger.warning(f"Warning: multiple calls queued with identical qid: {qid}. Most recent call will overwrite old call.")
...
class CoinGeckoAPIExtra(CoinGeckoAPI): 
    ...
    def __init__(self, *args, **kwargs): 
        ...
        super().__init__(*args, **{k: v for k, v in kwargs.items() if k not in filter_kwargs})
        ...
        if overriden_methods: 
            raise Exception(f"Wrapper class overwrote existing methods on CoinGeckoAPI class: {overriden_methods}")
